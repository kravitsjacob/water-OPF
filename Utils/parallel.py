...
def format_ticks(i, ax, n_ticks, limits, cols, flip_idx, x, tick_precision, explicit_ticks, label_fontsize, df, rotate_x_labels=False):
    # Format X axis
...
def static_parallel(df, columns, limits=None, columns_to_flip=None, n_ticks=6, tick_precision=None, explicit_ticks=None,
                    label_fontsize=12, plot_legend=False, plot_colorbar=False, colorbar_colormap='viridis',
                    figure_size=(11, 3), colorbar_adjust_args = (0.90, 0.2, 0.02, 0.70),
                    subplots_adjust_args = {'left': 0.05, 'bottom': 0.20, 'right': 0.85, 'top': 0.95, 'wspace': 0.0, 'hspace': 0.0}, rotate_x_labels=False):
    """
...
def main():
    ...
    static_parallel(df=df, columns=obj_labs, tick_precision=[4, 2, 1, -1])  # with user-specified number of ticks and precision
    static_parallel(df=df, columns=obj_labs, columns_to_flip=['Objective 1'])  # Flipping columns
    static_parallel(df=df, columns=obj_labs, limits=[[0, 0.2], [0, 0.6], [0, 0.6], [10, 50]])  # Setting user-specific column limits
